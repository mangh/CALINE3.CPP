...
{
    class CALINE3 : public ::testing::Test
    {
    protected:
        const char* input_data = R"sample(EXAMPLE FOUR                             60.100.   0.   0.12        1.
RECP. 1                  -350.       30.       1.8
RECP. 2                     0.       30.       1.8
RECP. 3                   750.      100.       1.8
RECP. 4                   850.       30.       1.8
...
LINK F              BR  -100.   200.  -100.  -200.   5000. 50. 6.1 27.
 ...
 1.270.6 1000. 6.7
        ...
        // Expected output data (mass concentrations; dimensionless)
        double mc[4][6][12] =
        {
            {
        ...
        CALINE3() :
            m_locale(std::setlocale(LC_ALL, "en_US.UTF-8")),
            m_input(std::istringstream(input_data)),
            m_reader(JobReader{ "INTERNAL DATA", m_input}),
            m_job_found(m_reader.Read())
        {
        }
    };

    TEST_F(CALINE3, ComputingConcentration)
    {
        if (!m_job_found)
        {
            FAIL() << "Could not read input data.";
        }

        const Job& site = m_reader.LastJob();
        ...
        for (auto const& meteo : site.Meteos)
        {
            for (auto const& link : site.Links)
            {
                ...
                for (auto const& receptor : site.Receptors)
                {
                    ...
                    EXPECT_DOUBLE_EQ(MC.value(), mc[meteo.ORDINAL][link.ORDINAL][receptor.ORDINAL])
                        << "  MC (mass concentration of the pollutant) = " << MC << std::endl
                        << "  - at the receptor: " << receptor << std::endl
                        << "  - from the link: " << link << std::endl
                        << "  - exposed to the meteo: " << meteo;
                }
            }
        }
    }
