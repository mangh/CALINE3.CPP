...
// // Expected ULP precision (higher on Windows, where test data was obtained)
// #ifdef _WIN32
// #   define ULPS 1
// #else
// #   define ULPS 2
                ...
                for (auto const& link : site.Links)
                {
                    ...
                    for (auto const& receptor : site.Receptors)
                    {
                        ...
                        REQUIRE_THAT(
                            ...
                            Catch::Matchers::WithinRel(mc.value(), 1.0e-15)
                        );

                        // REQUIRE_THAT(
                        //     mc.value(),
                        //     Catch::Matchers::WithinULP(test_result[meteo.ORDINAL][link.ORDINAL][receptor.ORDINAL], ULPS)
                        // );
                    }
                }
            }
