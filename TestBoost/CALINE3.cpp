...
LINK F              BR  -100.   200.  -100.  -200.   5000. 50. 6.1 27.
 ...
 1.270.6 1000. 6.7
    ...
    BOOST_FIXTURE_TEST_CASE(ComputingConcentration, Sample)
    {
        if (!m_job_found)
        {
            BOOST_FAIL("Could not read input data");
        }
        ...
        for (auto const& meteo : site.Meteos)
        {
            for (auto const& link : site.Links)
            {
                ...
                for (auto const& receptor : site.Receptors)
                {
                    ...
                    BOOST_CHECK_CLOSE_FRACTION(MC.value(), mc[meteo.ORDINAL][link.ORDINAL][receptor.ORDINAL], 1.0e-15);
                }
            }
        }
    }
